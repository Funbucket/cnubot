...
<section class="grid"><div class="metric"><span>고유 노출 사용자</span><b>{exposed:,}명</b><small>상품 기준 중복 제거</small>{details(summary_sql)}</div><div class="metric"><span>고유 클릭 사용자</span><b>{clicked:,}명</b><small>버튼·퀵리플라이·commerceCard</small>{details(summary_sql)}</div><div class="metric"><span>전체 CTR</span><b>{ctr:.2f}%</b><small>{totals.get('events') or 0:,}건의 이벤트 기록</small>{details(summary_sql)}</div></section>
