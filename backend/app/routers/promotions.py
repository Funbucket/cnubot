...
async def get_toss_shopping_promotion(req: KakaoRequest | None = Body(default=None)):
    ...
    try:
        product_key, product = await promotions.get_live_toss_product(user_id)
    except Exception:
    ...
    click_url = (
        ...
        f"{promotions.create_tracking_token(user_id, product_key, 'commerce_card', product.get('category_ids'), product.get('url'))}"
...
async def track_toss_shopping_click(token: str = Query(..., min_length=20)):
    ...
    user_id, product_key, _source, category_ids, target_url = decoded
    await recommendations.record_category_click(user_id, category_ids)
